...
int main()
{
    ...
    int n;
   int m[2001][2001];
   for(int c=0;c<2001;c++) for(int c2=0;c2<2001;c2++) m[c][c2]=0;
    cin>>n;
    ...
    for(int c=0;c<n;c++)
    {
       ...
       {
           if((v[c].first+v[c2].first)%2==0&&(v[c].second+v[c2].second)%2==0)
            {
                m[(v[c].first+v[c2].first)/2][(v[c].second+v[c2].second)/2]++;
            }
       }
    }
    int tot=0;
