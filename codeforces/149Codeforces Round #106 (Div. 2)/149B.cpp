...
int main()
{
    ...
    {
        int c;
        for(c=0;c<s.size()&&s[c]!=':';c++);
        s1=s.substr(0,c);
        s2=s.substr(c+1,s.size()-c-1);
    }
    map<char,int> m;
    for(int c=0;c<10;c++)
        m[c+'0']=c;
    ...
    for(int c=0;c<s1.size();c++)
    {
        ...
        base_max=max(base_max,m[s1[c]]);
    }
    for(int c=0;c<s2.size();c++)
        base_max=max(base_max,m[s2[c]]);
    ...
    for(int c=base_max+1;c<=59;c++)
    {
        ...
        for(int c2=0;c2<s2.size();c2++)
            tot2=tot2*c+m[s2[c2]];
        if(tot1>23||tot2>59||tot1<0||tot2<0)
        {
            if(c==base_max+1)
                cout<<0;
            break;
